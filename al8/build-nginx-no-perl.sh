...
_build_nginx() {
    ...
    _http_module_args="$((./configure --help 2>&1 || true) | grep -i '\--with-http' | awk '{print $1}' | sed 's/^[ ]*//g' | sed 's/[ ]*$//g' | grep -v '=' | sort -u | uniq | grep -ivE 'geoip|perl' | paste -sd' ')"
    _stream_module_args="$((./configure --help 2>&1 || true) | grep -i '\--with-stream' | awk '{print $1}' | sed 's/^[ ]*//g' | sed 's/[ ]*$//g' | grep -v '=' | sort -u | uniq | grep -ivE 'geoip|perl' | paste -sd' ')"
