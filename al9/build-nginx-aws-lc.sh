...
_build_nginx() {
    ...
    #rm -fr var/run
    rm -fr var
    [ -d usr/man ] && mv -vf usr/man usr/share/
    [ -d usr/lib64/perl5/5.32 ] && mv -vf usr/lib64/perl5 usr/lib64/perl5.old && mv -vf usr/lib64/perl5.old/5.32 usr/lib64/perl5 && rm -vfr usr/lib64/perl5.old
...
chmod 0644 etc/sysconfig/nginx
    ...
    sleep 1
    tar -Jcvf /tmp/nginx-"${_nginx_ver}"_"awslc${_aws_lc_tag/v/}"-1_el9_amd64.tar.xz *
    echo
    sleep 1
    cd /tmp
    openssl dgst -r -sha256 nginx-"${_nginx_ver}"_"awslc${_aws_lc_tag/v/}"-1_el9_amd64.tar.xz | sed 's|\*| |g' > nginx-"${_nginx_ver}"_"awslc${_aws_lc_tag/v/}"-1_el9_amd64.tar.xz.sha256
    rm -fr "${_tmp_dir}"
    rm -fr /tmp/nginx
    /sbin/ldconfig
}
...
echo
echo ' build nginx aws-lc al9 done'
echo
exit
